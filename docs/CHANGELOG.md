...
## 2026-06-14（追記7）
- **取得元の精査（15→19サイト）**。利用者の追加候補リストから、RSS有効・無料・非重複のものを厳選して追加し、下位互換を整理:
  - 追加: 企業テックブログまとめ（yamadashy・Google/MS等を横断）、はてなブックマーク テクノロジー、gihyo.jp 開発、CodeZine、InfoQ（日本語）。
  - 入れ替え: InfoQ（英・機械翻訳）→ InfoQ（日本語・公式）。同一媒体に日本語公式版があり英語版は下位互換のため削除。
  - 見送り: The Verge（消費者ガジェット寄りで関連薄）、WIRED（メータード課金＝有料の壁あり）、HN公式RSS（既存 hnrss と重複）、
    Awesome Tech RSS（単一フィードでなくOPML一覧）。クラウドWatchは前回404で見送り済み。
  - 確認結果: 現在の取得元に「有料でないと読めない」サイトは無し（課金ありのWIREDは追加候補側だったため不採用）。`web/data/sources.json` を19件に更新。
...
- **アプリアイコンを刷新**（案3「ネットワーク・ドーン」）: 紺の夜明け空＋情報源をつなぐシアンのネットワーク＋地平線から
  ...
  `web/icons/`（512/192/180/32）を再生成。旧 `tools/make_icons.py`（朝日アイコン生成）は廃止。
  Pillowはローカルのアイコン生成専用（requirements.txt には追加せず、Actions/実行環境では不要）。SWキャッシュ v8。
